...
public static class FigmaUIHelper
{
    ...
    /// </summary>
    public static Shadow ApplyShadow(GameObject target, Vector2 offset, Color color, bool useGraphicAlpha = true)
    {
    ...
    /// </summary>
    public static Outline ApplyOutline(GameObject target, Vector2 effectDistance, Color color, bool useGraphicAlpha = true)
    {
    ...
    /// </summary>
    public static void ApplyAutoLayout(GameObject target, string direction = "VERTICAL", float spacing = 0f, RectOffset padding = null)
    {
        ...
        if (direction.ToUpper() == "HORIZONTAL")
        {
            ...
            if (horizontalGroup == null) horizontalGroup = target.AddComponent<HorizontalLayoutGroup>();
            layoutGroup = horizontalGroup;
        }
        else
        {
            VerticalLayoutGroup verticalGroup = target.GetComponent<VerticalLayoutGroup>();
            if (verticalGroup == null) verticalGroup = target.AddComponent<VerticalLayoutGroup>();
        ...
        if (csf == null) csf = target.AddComponent<ContentSizeFitter>();
    ...
    /// </summary>
    public static GridLayoutGroup ApplyGridLayout(GameObject target, Vector2 cellSize, Vector2 spacing, int constraintCount = 1)
    {
        GridLayoutGroup grid = target.GetComponent<GridLayoutGroup>();
        if (grid == null) grid = target.AddComponent<GridLayoutGroup>();
