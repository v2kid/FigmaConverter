...
public class FigmaSerializationTester : MonoBehaviour
{
    ...
    public void TestAllNodeTypes()
    {
        ...
        {
            Debug.LogWarning($"⚠️ {failed} node type(s) have serialization issues that need to be addressed.");
        }
    }

    private TestResult TestNodeType<T>(string nodeTypeName) where T : FigmaNode, new()
    {
        try
        {
            ...
            {
                return new TestResult(nodeTypeName, false, $"Type mismatch: expected {typeof(T).Name}, got {deserializedNode.GetType().Name}");
            }
    ...
    private T CreateTestNode<T>() where T : FigmaNode, new()
    {
        T node = new T();
