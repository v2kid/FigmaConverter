...
public static class FigmaVectorHandler
{
    ...
    public static string GenerateSVGFromVector(JObject nodeData, float width, float height)
    {
        ...
        svg.AppendLine(
            $"<svg width=\"{totalWidth * scale}\" height=\"{totalHeight * scale}\" "
        ...
        else
        {
            ...
            {
                svg.AppendLine(
                    GenerateSVGPathsFromArray(vectorPaths, nodeData, width * scale, height * scale)
                );
            }
            else
            {
                ...
                svg.AppendLine(
                    FigmaShapeRenderer.GenerateSVGRectangle(nodeData, width * scale, height * scale)
                );
            }
        }
