...
public class NodeSpriteGenerator
{
    ...
    public static Sprite GenerateSpriteFromNode(
        JObject nodeData,
        float width,
        float height,
        bool saveToResources = false
    )
    {
        try
        {
    ...
    public static string GenerateSVGFromNode(JObject nodeData, float width, float height)
    {
        ...
        svg.AppendLine(
            $"<svg width=\"{totalWidth * SVG_SCALE}\" height=\"{totalHeight * SVG_SCALE}\" "
        ...
        {
            svg.AppendLine(
                $"<g transform=\"translate({effectPadding * SVG_SCALE}, {effectPadding * SVG_SCALE})\">"
            );
        }
